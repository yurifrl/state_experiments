...
import 'package:reactive_exploration/src/catalog/catalog_slice.dart';
import 'package:reactive_exploration/src/services/catalog.dart';
import 'package:reactive_exploration/src/services/catalog_page.dart';
...
class CatalogBloc {
  ...
  final _sliceSubject = BehaviorSubject<CatalogSlice>.seeded(CatalogSlice.empty());
  ...
  void _handleIndexes(List<int> indexes) {
    ...
    for (int i = minPageIndex; i <= maxPageIndex; i += CatalogService.productsPerPage) {
    ...
    _pages.removeWhere(
        (pageIndex, _) => pageIndex < minPageIndex - CatalogService.productsPerPage || pageIndex > maxPageIndex + CatalogService.productsPerPage);
  }
...
class CatalogProvider extends InheritedWidget {
  ...
  static CatalogBloc of(BuildContext context) => (context.dependOnInheritedWidgetOfExactType<CatalogProvider>() as CatalogProvider).catalogBloc;
}
